...
Inflate := [].{
	...
	inflate_block = |input, in_next0, bitbuf0, bitsleft0, overread0, out0, tables| {
		...
		while $done == 0 and $in_next + 8 <= in_len {
			...
			var $entry = List.get_unchecked(litlen_table, $bitbuf.bitwise_and(litlen_mask))
			...
			if $entry.bitwise_and(Inflate.huffdec_subtable_pointer) != 0 {
				...
				$entry = List.get_unchecked(litlen_table, sub_index)
				$saved_bitbuf = $bitbuf
				$consumed = $entry.bitwise_and(255).to_u64()
				$bitbuf = $bitbuf.shr_zf_wrap($consumed.to_u8_wrap())
				$bitsleft = $bitsleft - $consumed
			...
			if $entry.bitwise_and(Inflate.huffdec_literal) != 0 {
				...
				$entry = List.get_unchecked(litlen_table, $bitbuf.bitwise_and(litlen_mask))
				$saved_bitbuf = $bitbuf
				$consumed = $entry.bitwise_and(255).to_u64()
				$bitbuf = $bitbuf.shr_zf_wrap($consumed.to_u8_wrap())
				$bitsleft = $bitsleft - $consumed
				if $entry.bitwise_and(Inflate.huffdec_subtable_pointer) != 0 {
					...
					$entry = List.get_unchecked(litlen_table, si2)
					$saved_bitbuf = $bitbuf
					$consumed = $entry.bitwise_and(255).to_u64()
					$bitbuf = $bitbuf.shr_zf_wrap($consumed.to_u8_wrap())
					$bitsleft = $bitsleft - $consumed
				...
				if $entry.bitwise_and(Inflate.huffdec_literal) != 0 {
					...
					if $entry.bitwise_and(Inflate.huffdec_subtable_pointer) != 0 {
						...
						$entry = List.get_unchecked(litlen_table, si3)
						$saved_bitbuf = $bitbuf
						$consumed = $entry.bitwise_and(255).to_u64()
						$bitbuf = $bitbuf.shr_zf_wrap($consumed.to_u8_wrap())
						$bitsleft = $bitsleft - $consumed
			...
			if $pending == 1 {
				if $entry.bitwise_and(Inflate.huffdec_end_of_block) != 0 {
					...
					var $off_entry = List.get_unchecked(offset_table, $bitbuf.bitwise_and(255))
					if $off_entry.bitwise_and(Inflate.huffdec_exceptional) != 0 {
						...
						$off_entry = List.get_unchecked(offset_table, osi)
					} else {}
